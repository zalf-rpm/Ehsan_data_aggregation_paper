...
def run_consumer(leave_after_finished_run = True, server = {"server": None, "port": None}, shared_id = None):
    ...
    #count cols in rows
    datacells_per_row = 0#np.sum(soil_grid_template, axis=1)

    start_row = 0#int(config["start-row"])
    end_row = -1#int(config["end-row"])
    ...
    def process_message(msg):
        ...
        if not write_normal_output_files:
            ...
            with open(path_to_out_dir + "soil_id-" + str(soil_id) + ".csv", "w", newline='') as _:
